...
public class Parameters extends ParametersForLbjCode{
	...
	public static void readConfigAndLoadExternalData(String configFile) throws Exception{
        ...
        if(!st.nextToken().equals("pathToTrainDevTest"))
            throw new Exception("Error- expecting the 'pathToTrainDevTest' parameter, read:"+line);
        pathToTrainDevTest=st.nextToken();
        // Make sure we have the '/' at the end
        if(pathToTrainDevTest.charAt(pathToTrainDevTest.length()-1) != '/'){
            pathToTrainDevTest += "/";
        }
